...
let
  features = rec {
    ...
    home-manager-users =
      ...
      in if builtins.pathExists f then import f else [ "tak_1" ];
    # =========================================================================
    # =========================================================================
    # 👤 SYSTEM USERS
    # =========================================================================
    # Per-user NixOS modules live in /etc/nixos/users/<name>.nix.
    # They are generated by the Tak_OS installer (scripts/install.sh).
    # Edit them directly after installation, then run: sudo nixos-rebuild switch
    #
...
in
{
  ...
  imports =
   [
     ...
     ./users-declared/tak_1.nix
     ./users-declared/tak_2.nix
